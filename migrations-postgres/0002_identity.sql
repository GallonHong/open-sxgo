...
-- row remains readable by the closed demo; this migration never turns legacy
-- roles or wildcard scopes into active grants.
...
CREATE INDEX IF NOT EXISTS principal_identities_person
  ON principal_identities (person_id, status);
...
CREATE INDEX IF NOT EXISTS principal_accounts_principal
  ON principal_accounts (principal_id, status);
...
CREATE INDEX IF NOT EXISTS principal_keys_active
  ON principal_keys (principal_id, purpose, status);

-- role_grants is created before principal_api_credentials because PostgreSQL
-- requires a referenced table to exist when a foreign key is declared.
CREATE TABLE IF NOT EXISTS role_grants (
  ...
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL,
  CHECK (capabilities::jsonb IS NOT NULL),
  CHECK (scope::jsonb IS NOT NULL),
  CHECK (position('*' IN capabilities) = 0),
  CHECK (position('"*"' IN scope) = 0)
);
CREATE INDEX IF NOT EXISTS role_grants_lookup
  ON role_grants (principal_id, status, expires_at);
CREATE INDEX IF NOT EXISTS role_grants_person
  ON role_grants (person_id, status, expires_at);
...
CREATE TABLE IF NOT EXISTS webauthn_credentials (
  ...
  user_verified_required BOOLEAN NOT NULL DEFAULT TRUE CHECK (user_verified_required = TRUE),
...
CREATE INDEX IF NOT EXISTS webauthn_credentials_principal
  ON webauthn_credentials (principal_id, status);
...
-- session-bound proof stored below.
...
CREATE INDEX IF NOT EXISTS webauthn_challenges_active
  ON webauthn_challenges (session_id, expires_at, consumed_at);
...
CREATE INDEX IF NOT EXISTS session_assurance_current
  ON session_assurance (session_id, expires_at);
...
CREATE INDEX IF NOT EXISTS conflict_declarations_lookup
  ON conflict_declarations (person_id, status, object_type, object_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS case_assignments_active_person
  ...
  WHERE status IN ('assigned', 'accepted', 'in_progress');
CREATE INDEX IF NOT EXISTS case_assignments_person
  ON case_assignments (person_id, status, expires_at);
...
CREATE TABLE IF NOT EXISTS authorization_jobs (
  ...
  updated_at TEXT NOT NULL,
  closed_at TEXT,
  CHECK (scope_snapshot::jsonb IS NOT NULL)
);
CREATE INDEX IF NOT EXISTS authorization_jobs_queue
  ON authorization_jobs (state, updated_at);
CREATE INDEX IF NOT EXISTS authorization_jobs_grant
  ON authorization_jobs (grant_id, state);
...
CREATE INDEX IF NOT EXISTS outbox_events_ready
  ON outbox_events (state, available_at);
...
CREATE TABLE IF NOT EXISTS identity_legacy_import (
  ...
  migration_status TEXT NOT NULL DEFAULT 'demo_only'
    CHECK (migration_status IN ('demo_only', 'pending_reverification', 'imported')),
  imported_at TEXT NOT NULL,
  reviewed_at TEXT,
  reviewed_by TEXT
);

INSERT INTO identity_legacy_import
  (user_id, person_id, legacy_roles, legacy_company_ids, legacy_conflicts, migration_status, imported_at)
SELECT user_id, person_id, roles, company_ids, conflicts, 'demo_only',
