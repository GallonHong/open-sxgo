...
CREATE TABLE IF NOT EXISTS contributions (
  ...
  status TEXT NOT NULL CHECK (status IN (
    'submitted', 'deduplicated', 'under_assessment', 'accepted_pending',
    'recognized', 'duplicate', 'needs_info', 'not_qualified', 'challenged',
...
CREATE INDEX IF NOT EXISTS contributions_principal
  ON contributions (principal_id, domain, status);
CREATE INDEX IF NOT EXISTS contributions_cluster
  ON contributions (contribution_cluster_id, status);
...
CREATE INDEX IF NOT EXISTS contribution_assessor
  ON contribution_assessments (contribution_id, created_at);
...
CREATE INDEX IF NOT EXISTS qualification_principal
  ON qualification_applications (principal_id, target_role, status);
...
CREATE INDEX IF NOT EXISTS appeals_subject
  ON contribution_appeals (subject_type, subject_id, status);
...
CREATE TABLE IF NOT EXISTS governance_proposals (
  ...
  state TEXT NOT NULL CHECK (state IN (
    'draft', 'discussion', 'voting', 'passed', 'rejected', 'withdrawn',
    'expired', 'timelocked', 'ready_to_execute', 'executed', 'blocked', 'failed'
  )),
...
CREATE INDEX IF NOT EXISTS governance_proposal_state
  ON governance_proposals (state, updated_at);
...
CREATE INDEX IF NOT EXISTS electorate_snapshot_lookup
  ON electorate_snapshots (proposal_id, frozen_at);
...
CREATE INDEX IF NOT EXISTS governance_ballots_latest
  ON governance_ballots (proposal_id, principal_id, ballot_revision);

-- P0 uses a three-person transition committee.  Mature community ballots
-- remain separate and disabled until the P1 governance migration.
...
CREATE INDEX IF NOT EXISTS governance_committee_approval_lookup
  ON governance_committee_approvals (proposal_id, proposal_revision, decision);
...
CREATE INDEX IF NOT EXISTS governance_nodes_status
  ON governance_nodes (status, valid_until);
