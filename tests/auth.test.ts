...
import * as pgSchema from '../packages/db/src/pg-schema';
...
for (const adapter of process.env.TEST_POSTGRES_URL ? ['sqlite', 'postgres'] : ['sqlite'])
  it(adapter + ' 实际 Better Auth 密码 + TOTP 注册和强制登录', async () => {
    const local = adapter === 'sqlite' ? openDatabase(':memory:') : undefined;
    const pg = adapter === 'postgres' ? await testDatabase('postgres') : undefined;
    const db = local?.db ?? pg!.db;
    try {
      if (local) await migrate(local.sqlite);
      const now = Date.now(),
      ...
      const auth = createAuth(
        local ? drizzle(local.sqlite, { schema }) : drizzlePg(pg!.pool!, { schema: pgSchema }),
        'test-only-long-secret-not-production-1234567890',
        'http://localhost:8787',
        undefined,
        adapter === 'postgres' ? 'pg' : 'sqlite',
      );
      ...
      expect(
        (await post('sign-up/email', { email: 'other@example.org', password, name: 'Other' }))
          .status,
      ).not.toBe(200);
      expect((await post('sign-in/email', { email: 'test@example.org', password })).status).toBe(
        200,
      );
