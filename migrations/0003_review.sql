...
-- case_assignments; this migration stores only the opaque assignment id and
-- never recreates that table.
...
CREATE TABLE IF NOT EXISTS sanitized_previews(
  ...
  image_ref TEXT,
  raw_html_available_to_reviewer INTEGER NOT NULL DEFAULT 0 CHECK(raw_html_available_to_reviewer = 0),
...
-- independent decisions must be recorded.  The candidate body is expected
-- to be a public-candidate projection, never a raw submission.
...
-- assignment_id is supplied and checked by the identity service.  Keeping
-- it opaque here prevents this workflow from becoming a second identity DB.
