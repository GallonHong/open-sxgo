...
export function pwa(): Plugin {
  return {
    ...
    generateBundle(_options, bundle) {
      ...
      const code = `const CACHE='wfd-shell-${version}';const ASSETS=${JSON.stringify(['/', '/index.html', ...assets])};self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(ASSETS))));self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(ks=>Promise.all(ks.filter(k=>k.startsWith('wfd-shell-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));self.addEventListener('fetch',e=>{const u=new URL(e.request.url);if(e.request.method!=='GET'||u.origin!==self.location.origin||u.pathname.startsWith('/public/')||u.pathname.startsWith('/private/')||u.pathname.startsWith('/admin')||['/contribute','/receipt','/report'].includes(u.pathname)||['/member/','/review/','/governance/'].some(p=>u.pathname.startsWith(p)))return;if(e.request.mode==='navigate'){e.respondWith(fetch(e.request).catch(()=>caches.match('/index.html')));return;}if(ASSETS.includes(u.pathname))e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request)));});`;
